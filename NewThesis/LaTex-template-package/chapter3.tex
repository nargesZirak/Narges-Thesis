...
\RW{I've rethought how this can work.  You can separate theoretical background on AU and ASTs from background on JDT and Jigsaw from your *new* demonstration of the behaviour of Jigsaw and from the *new* material on AUASTs including a demonstration of its behaviour.  This will give us four short chapters.}
...
% JDT
\RW{Explain what an AST is before talking about details of the JDT.  Show us tree-based representations of code.  Then show us JDT-based representations. In my reworked example, I have used production names as per the standard JLS grammar (as the function symbols).  This stuff about child node properties belongs later.}
...
\RW{This is a concrete implementation, not a generic idea, at least not the way it is described.  I strongly suggest that you give a generic description of your assumptions about ASTs then relate AUASTs to those, then talk about implementation details.  But the reality is that the AUAST is *your* idea, not Cottrell's---take credit for it.  It should be another short chapter after the background.  You can add ``experiments'' (description of test suite and its results) there that shows that your approach works.}
