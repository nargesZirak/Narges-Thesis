...
\NZ{In the AST section, I explained how an instance of an AST can be represented in an abstract form that can be mapped to the definition of a term, so it does not make sense if it comes before the definition of a term!}
...
Unification usually aims to create the \emph{most general unifier} (MGU), that is, $U$ is the MGU of two terms such that for all unifiers $U'$ there exists a substitution $\Theta$ such that $U\xrightarrow{\Theta}U'$. Unification has been used for various applications, however, it is not helpful to solve our problem as we need to construct more generalized structures. \RW{The following description does not justify the claim that unification is not helpful.}
\NZ{Does it make sense now? as I noted we need to construct more generalized structures, that is why we need the inverse of unification}
