...
\end{lstlisting}
\caption{A simple example \protect\tool{Java} program.\label{fig:java-example}}
\end{figure}
...
\end{lstlisting}
\caption[Example 1: A \protect\tool{Java} method that uses a log statement.]{A Java method that uses a log statement. This will be referred to as Example 1.\label{ch3-ex1}}
\end{figure}
...
\end{lstlisting}
\caption[Example 2: A \protect\tool{Java} method that uses a log statement.]{A Java method that uses a log statement. This will be referred to as Example 2.\label{ch3-ex2}}
\end{figure}
...
\begin{defn}[Applying a First-Order Substitution]\label{def:apply_substitution}
...
\begin{defn}[First-Order Anti-unifier (and Unifier)]\label{def:generalization}
...
\node(right) at (8,0) {\fbox{\mbox{$f(a,Y)$}}};
\node(u) at (5,-2) {\parbox{2cm}{\begin{center}\fbox{$f(a,b)$}\\*[3pt]\text{unified term}\end{center}}};
%
...
\draw[->](left) -- (u) node[pos=0.5, below]{\mbox{\scriptsize $\sigma' = \{X \mapsto a, Y \mapsto b\}$\hspace*{3cm}}};
\draw[->](right) -- (u) node[pos=0.5, below]{\mbox{\hspace*{3cm}\scriptsize $\sigma' = \{X \mapsto a, Y \mapsto b\}$}};
\end{tikzpicture}
...
\begin{defn}[Higher-Order Substitution]\label{def:HOsubstitution}
...
\begin{defn}[Applying a Higher-Order Substitution]\label{def:apply_HOsubstitution}
...
\begin{defn}[Higher-Order Anti-unifier (and Unifier)]\label{def:HOgeneralization}
The substitution $\hat{\sigma}$ is a \emph{higher-order unifier} for the higher-order terms $\hat{t}_1$ and $\hat{t}_2$ if and only if %there exist higher-order substitutions $\hat{\sigma}_1'$ and $\hat{\sigma}_2'$ such that
%$\hat{\sigma}_1'(t_1)=u$ and $\hat{\sigma}_2'(t_2)=u$.
...
I have also defined a set of equational theories to incorporate semantic knowledge of structural equivalences supported by the \name{Java} language specification, as it provides various syntactic ways to define semantically equivalent structures.
...
For example, consider \code{for}- and \code{while}-statements that are two kinds of looping structure in \name{Java}: they have different syntax but cover the same concept. To be able to anti-unify these structures meaningfully, we need an equational theory that can equate any \code{for}-loop
\begin{center}
\code/for(/$\id{inits}$\code/; /$\id{test}$\code/; /$\id{updates}$\code/){...}/
\end{center}
to an equivalent \code{while}-loop
\begin{center}
%\mbox{
...
This notion of pumping is equivalent to defining an equational theory $=_{\id{pump}}$ such that
\begin{gather*}
\id{pump}_n(\nu_{\id{pump}_n}, \ldots, \nu_{\id{pump}_n}, t, \nu_{\id{pump}_n}, \ldots, \nu_{\id{pump}_n})\\
...
For simplicity, we treat \code{for}- and \code{while}-loops as functions with differing arguments, defining an equivalence equation $=_{\id{loops}}$ that allows their direct anti-unification. We then utilize the \NIL{}-theory to handle the varying number of arguments as the \code{for}-loop has three arguments (actually, all three are optional in practice) whereas the \code{while}-loop only has one. Using the \NIL{}-theory we can create the structure \code{while}(\NIL{}(\NIL{}, \NIL{}), \textit{lessThanExpression}(\code{i}, \code{10}), \NIL{}(\NIL{}, \NIL{})) that is $=_{\id{loops}}$ to \code{while}(\textit{lessThanExpression}(\code{i}, \code{10})) and construct the anti-unifier, $V_0$($V_1$($V_2$, $V_3$), \textit{lessThanExpression}(\code{i}, \code{10}), $V_4$($V_5$($V_2$))), as depicted in Figure~\ref{fig:for-while}.
...
%In practice, anti-unification of \code{for}- and \code{while}-loops is siby converting the \code{for}-loop to the equivalent \code{while}-loop and anti-unifying the result directly, which can find correspondences between the initialization expression and equivalent assignment statements prior to the \code{while}-loop, and between the update expression and equivalent statements, typically at the end of the block in the \code{while}-loop.
...
%or the most representative object of a cluster
...
%Each object is assigned to a centroid is a cluster. The centroid of each cluster is then updated based on the objects assigned to the cluster.  repeatedly perform the assignment and update operations until the centroids do not change.
