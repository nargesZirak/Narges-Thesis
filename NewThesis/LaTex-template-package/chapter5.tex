...
\\\midrule
 ...
 Clusters  & \makebox[\Ac][c]{\makebox[\A][r]{29}}                 & \makebox[\Bc][c]{\makebox[\B][r]{7}}                 & \makebox[\Abc][c]{\makebox[\Cl][r]{24}} & \makebox[\Abc][c]{\makebox[\Cl][r]{}}\\


  Generalized clusters  & \makebox[\Ac][c]{\makebox[\A][r]{14}}                 & \makebox[\Bc][c]{\makebox[\B][r]{4}}                 & \makebox[\Abc][c]{\makebox[\Cl][r]{9}}  & \makebox[\Abc][c]{\makebox[\Cl][r]{}} \\


  Singleton clusters       & \makebox[\Ac][c]{\makebox[\A][r]{15}}                 & \makebox[\Bc][c]{\makebox[\B][r]{3}}                 & \makebox[\Abc][c]{\makebox[\Cl][r]{13}} & \makebox[\Abc][c]{\makebox[\Cl][r]{}} \\\midrule

  Reduction                       & \makebox[\Ac][c]{\makebox[\A][r]{97\%\hspace*{-\Pwa}}} & \makebox[\Bc][c]{\makebox[\B][r]{94\%\hspace*{-\Pwa}}} &\makebox[\Bc][c]{\makebox[\B][r]{96\%\hspace*{-\Pwa}}}&\makebox[\Bc][c]{\makebox[\B][r]{\hspace*{-\Pwa}}}\\
...
\begin{sidewaysfigure} [p]
  \subfloat[]{\includegraphics[trim={0.2cm 9cm 18cm 0.5cm},clip,width=0.5\textwidth]{Charts/Histograms.png}}\quad
  \subfloat[]{\includegraphics[trim={18cm 9cm 0.2cm 0.5cm},clip,width=0.5\textwidth]{Charts/Histograms.png}}\\
  \subfloat[]{\includegraphics[trim={0.2cm 0 18cm 8.5cm},clip,width=0.5\textwidth]{Charts/Histograms.png}}\quad
  \subfloat[]{\includegraphics[trim={18cm 0 0.2cm 8.5cm},clip,width=0.5\textwidth]{Charts/Histograms.png}}
...
The main common structural characteristics of the anti-unifiers of this category are the \code{try} and \code{catch} clauses, where the \code{log statements} are located inside the body of a \code{catch} clause. As shown in Figure~\ref{fig:categories}, \% to \% of the total LMs are described by the anti-unifiers of this category, and it is the most commonly used logging usage category in the \name{Tomcat} and \name{Hibernate} software systems. The popularity of this category among all the studied systems is not surprising given the fact that exception handling using \code{try} statements is a common error handling technique in the Java programming language.
...
In this category, log statements are enclosed by \code{if} statements using expressions that mostly consist of \textsl{infixExpression}, \textsl{methodInvocation}, or \textsl{binaryExpression} nodes. The \textsl{infixExpression}s mostly either check the equality of an expression with \code{null} or tests the validity of the value of a variable; the \code{if} statements involving \textsl{methodInvocation}s mostly check if the return value of an invoked method is an indicator of a potential problem within the system; and the \code{if} statements involving \textsl{binaryExpression}s mostly check if a \textsl{booleanLiteral} is incorrect. As shown in Figure~\ref{fig:categories}, ..\% to \% of the total LMs are described by the anti-unifiers of this category, and it is the most commonly used logging category in the \name{...} system.

\subsubsection{\emph{C. Method Logging}}  \label{method logging}
In this category, the log statements are located inside the body of \textsl{methodDeclaration} nodes. A common structural characteristic of the anti-unifiers in this category is that they mostly use \code{throwStatement}s to throw an exception if an error occurs. The percentage of LMs that are described by the anti-unifiers of this category ranges from \% to \%, and it is the most common logging usage category in the \name{...}. This suggests that developers use logging to record important method granularity information about the state of a software system. This information might be used later to detect the root causes of an application problem.
% %?
% throw exception
